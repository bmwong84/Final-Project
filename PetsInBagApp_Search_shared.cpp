...
int main()
{
	...
	while (choice != 0)  // not a bullet proof data entry, enhancements welcome
	{
		...
		case 1:  
			...
			try 
			{
				bag.addPet(newPet); // Polymorphism: addPet is defined to add generic Pet_T, but passes in Cat_T
			}
			catch (Bag_E e) 
			{
				e.handleError();
		...
		case 2: 			
			...
			try 
			{
				bag.addPet(newPet); // Polymorphism: addPet is defined to add generic Pat_T, but passes in Dog_T
			}
			catch (Bag_E e) 
			{
				e.handleError();
	...
	Bag_T bag1 = Bag_T(2);
	bag1.addPet(new Cat_T("Fluffy", "123", "Furr loss"));
	bag1.addPet(new Cat_T("Sleepy", "345", "Blind eye"));
	cout << "Content of Bag1:" << endl;
	cout << bag1;

	Bag_T bag2 = Bag_T(3);
	bag2.addPet(new Dog_T("Rufus", "560", "Heartworms"));
	bag2.addPet(new Dog_T("Sasha", "777", "Hip Dysplasia"));
	bag2.addPet(new Dog_T("Misha", "555", "Broken Leg"));
