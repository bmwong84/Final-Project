...
namespace csci2312
{
	...
	typedef std::shared_ptr<Pet_T> sharedPetPtr;
	...
	bool sortPetPredicate(sharedPetPtr left, sharedPetPtr right);
	...
	bool sharedSearchPetPredicate(sharedPetPtr left, sharedPetPtr right);
	//{
	...
	class Dog_T : public Pet_T {
		public:
			...
			Dog_T(string tag, string name, string illness);
	...
	class Bag_T {
	  public:
		  ...
		  void addPet(sharedPetPtr) throw (Bag_E);
		  //void readFile();
		  ...
		  template<typename PetType_T> void releasePets()
		  {
			  ...
			  //removes the generic pet from the bag, whether dog or cat
			  PetType_T* petPointer = nullptr;
			  for (unsigned count = 0; count < sharedPets.size(); count++)
			  {
				  petPointer = dynamic_cast<PetType_T*>(sharedPets[count]);
				  if (petPointer != nullptr)
				  {
					  //pet speak petPointer->???  make generic speak?
					  delete sharedPets[count];
					  sharedPets.erase(sharedPets.begin() + count);
					  count--;
				  }
			  }
		  };
	...
	private:
		...
		vector<sharedPetPtr>::iterator sharedWhere;		//for smart ptr
